...
class ImageResource(Base):
    ...
    original_dicom_tags = Column(JSON, nullable=True, comment="原始 DICOM 元数据")
    created_at = Column(DateTime, nullable=True, comment="创建时间")


class AITask(Base):
    """AI 推理任务表，记录一次多模态推理请求的上下文数据。"""
    __tablename__ = "ai_tasks"

    task_id = Column(String(50), primary_key=True, comment="任务唯一标识")
    image_uid = Column(String(50), nullable=True, index=True, comment="关联的图像资源 ID (可为空)")
...
class AIFeature(Base):
    """AI 推理结果特征表，存储最终生成的结构化诊断报告。"""
    __tablename__ = "ai_features"

    task_id = Column(String(50), primary_key=True, comment="关联的任务 ID")
    image_uid = Column(String(50), nullable=True, index=True, comment="关联的图像资源 ID")
